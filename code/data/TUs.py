...
class TUsDataset(torch.utils.data.Dataset):
    def __init__(self, name):
        ...
        self.train = [self.format_dataset([self.dataset[idx] for idx in self.all_idx['train'][split_num]]) for split_num in range(10)]
        self.val = [self.format_dataset([self.dataset[idx] for idx in self.all_idx['val'][split_num]]) for split_num in range(10)]
        self.test = [self.format_dataset([self.dataset[idx] for idx in self.all_idx['test'][split_num]]) for split_num in range(10)]
        
        print("Time taken: {:.4f}s".format(time.time()-t0))
    
    def _load_dataset(self):
        dataset_dir = os.path.join(os.path.expanduser('~'), '.dgl', self.name)
        if os.path.exists(dataset_dir):
            print(f"Using local dataset: {dataset_dir}")
            raw_dir = os.path.dirname(dataset_dir)
            dataset = CustomLegacyTUDataset(self.name, raw_dir=raw_dir)
        else:
            print(f"Local dataset not found. Attempting to download {self.name}...")
            dataset = CustomLegacyTUDataset(self.name)
    ...
    def format_dataset(self, dataset):  
        ...
        for graph in graphs:
            ...
            if 'feat' not in graph.edata.keys():
                edge_feat_dim = graph.ndata['feat'].shape[1]
